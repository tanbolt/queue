...
class Fqueue
{
    ...
    /**
     * 设置单个数据分片允许的最大文件尺寸（单位：MB），最大可设置为 1024M
     * @param int $partitionSize
     * @return $this
     */
    public function setPartitionSize(int $partitionSize)
    {
        $this->partitionSize = max(1, min(1024, $partitionSize));
        return $this;
    }
     ...
     * @return $this
     */
    public function setLabelSize(int $labelSize)
    {
     ...
     * @return bool|string
     */
    private function getTopicPath(string $topic, bool $create = false, int $label = null, $tryTimes = 0)
    {
        $topic = null === $label ? $topic : $topic.'_'.$label;
        if (array_key_exists($topic, $this->dirCheckCache)) {
            return $this->dirCheckCache[$topic];
        }
        clearstatcache();

        // 当前正在使用的 label 文件夹刚好写满, 正在修改 label 文件名备份, 等待修改完成后继续
        if ($create && is_file($lockFile = $this->folder . '/' . $topic . '.lock')) {
            if ($tryTimes > 500) {
                throw new CreateFailedException(sprintf('[%s] task is locked by file [%s]', $topic, $lockFile));
            }
            usleep(10000);
            return $this->getTopicPath($topic, $create, $label, $tryTimes + 1);
        }

        $path = $this->folder . '/' . $topic;
        if (!($exist = is_dir($path)) && $create) {
            // create dir
            if (false === @mkdir($path, 0777, true)) {
                throw new CreateFailedException(sprintf('Unable to create queue directory [%s]', $path));
            } else {
                $exist = true;
                $fp = fopen($path . '/label', 'wb');
                fwrite($fp, pack('l', $label ?: 0));
                fclose($fp);
            }
        }
        return $exist ? $this->dirCheckCache[$topic] = $path = realpath($path) : false;
    }
     ...
     * @return $this
     */
    private function storeTopicDelay(string $topic, array $messages, int $time = 1000)
    {
        $topicPath = $this->getTopicPath($topic, true);

        // 若当前刚好在重建延迟消息, 等待重建进程结束
        clearstatcache();
        if (is_file($rebuildPath = $topicPath. '/delayRebuild')) {
            ...
            usleep($time);
            return $this->storeTopicDelay($topic, $messages, $time * 2);
        }
        ...
        return empty($pack) ? $this : $this->writeTopicDelay($topic, $topicPath, $pack);
    }

    /**
     * 将延迟消息存储到 delayMessage 文件中
     * @param string $topic
     * @param string $topicPath
     * @param string $pack
     * @return $this
     */
    private function writeTopicDelay(string $topic, string $topicPath, string $pack)
    {
        $path =  $topicPath . '/delayMessage';
        if (!is_file($path)) {
            if (!file_put_contents($path, pack('l', 4))) {
                throw new IoException(sprintf('Failed to write delay message [%s]', $path));
            }
        }
        if (!($fp = $this->getWriteHandler($topic, self::HANDLER_DELAY_MESSAGE, 'ab', $path)) ||
            fseek($fp, 0, SEEK_END) < 0 || !fwrite($fp, $pack)
        ) {
            throw new IoException(sprintf('Failed to write delay message [%s]', $path));
        }
        return $this;
    ...
    private function writeTopicQueue(string $topic, array $messages)
    {
        ...
        // 打开当前分片索引文件
        $index = $this->getWriteHandler($topic, self::HANDLER_INDEX, 'a+b', $indexFile);
        if (!$index || !flock($index, LOCK_EX) || fseek($index, 0, SEEK_END) < 0 || 0 !== ($size = ftell($index)) % 4) {
            if ($index) {
                @flock($index, LOCK_UN);
            }
        ...
        // 打开 dat 文件，并获取已偏移量（即 size 大小）
        $dat = $this->getWriteHandler($topic, self::HANDLER_DAT, 'ab', $datFile);
    ...
    /**
     * 获得 topic 主题当前的分片文件的路径名
     * @param string $topic
     * @param int $padCount
     * @param ?int $label
     * @return string
     */
    private function getCurrentPartition(string $topic, int $padCount = 1, int $label = null)
    {
        $topicPath = $this->getTopicPath($topic, true, $label);
        ...
        // 当前 label 下不存在索引文件，为第一条消息，直接返回首个 dat 路径名
        ...
        // 从索引文件获取当前 dat 路径名
        $partitionIndex = $this->getWriteHandler($topic, self::HANDLER_PARTITION_INDEX, 'r+b', $path);
    ...
    public function getQueue(int $offset = 0, int $limit = 1, string $topic = self::DEFAULT_TOPIC)
    {
        ...
        if ($offset) {
            $fp = $this->getReadHandler($topic, self::HANDLER_PARTITION_INDEX, 'rb', $partition);
            $low = 0;
            $high = $max;
            while ($low <= $high) {
    ...
    private function pushDelayMessage(string $topic = self::DEFAULT_TOPIC)
    {
        ...
        // 打开存储延迟消息的文件
        $delay = $this->getReadHandler($topic, self::HANDLER_DELAY_MESSAGE, 'r+b', $path);
    ...
    {
        if (!$fp && (!$fp = $this->getReadHandler($topic, self::HANDLER_PARTITION_INDEX, 'rb', $partition))) {
    ...
    private function getPartitionMessage(
        ...
        $datFile = $path . '.dat';
        if (!is_file($datFile) || !($dat = $this->getReadHandler($topic, self::HANDLER_DAT, 'r', $datFile))) {
            throw new FileErrorException(sprintf('Data file [%s] not exist', $datFile));
        }
        if (!($index = $this->getReadHandler($topic, self::HANDLER_INDEX, 'r', $indexFile))) {
            throw new FileErrorException(sprintf('Index file [%s] is error', $indexFile));
        }
    ...
    private function popMessage(string $topic = self::DEFAULT_TOPIC)
    {
        ...
        $fp = $this->getReadHandler($topic, self::HANDLER_CURRENT, 'r+b', $path);
        flock($fp, LOCK_EX);
        fseek($fp, 0, SEEK_SET);
        $current = fread($fp, 4);
        if (empty($current)) {
    ...
    public function maxOffset(string $topic = self::DEFAULT_TOPIC, bool $fromStart = false)
    {
        ...
        $cache = $this->getReadHandler($topic, self::HANDLER_PARTITION_INDEX, 'rb', $path);
    ...
    public function currentOffset(string $topic = self::DEFAULT_TOPIC, bool $fromStart = false)
    {
        ...
        $current = false;
        if (($fp = $this->getReadHandler($topic, self::HANDLER_CURRENT, 'r+b', $path)) && fseek($fp, 0, SEEK_SET) > -1) {
            $current = fread($fp, 4);
            $current = @unpack('l', $current);
        }
        if (!is_array($current) || !isset($current[1])) {
     ...
     * @return resource
     */
    private function getWriteHandler(string $topic, string $handler, string $flag, string $path)
    {
        return $this->getResourceHandler($topic, $handler, $flag, $path, false);
    }
    ...
    {
        $this->closeResourceHandler($topic, false, $handler);
        return $this;
    }
     ...
     * @return resource
     */
    private function getReadHandler(string $topic, string $handler, string $flag, string $path)
    {
        return $this->getResourceHandler($topic, $handler, $flag, $path, true);
    }
    ...
    {
        $this->closeResourceHandler($topic, true, $handler);
        return $this;
    }
     ...
     */
    private function getResourceHandler(string $topic, string $handler, string $flag, string $path, bool $read = false)
    {
     ...
     * @return bool
     */
    private function closeResourceHandler(string $topic = null, bool $read = false, string $handler = null)
    {
        $type = $read ? 'read' : 'write';
        if (!isset($this->resourceHandler[$type])) {
            return true;
